...
```
**Ans: In this data set, we have 959 persons and 8348674 SNPs.**
...
```
**Ans: In this data set, we have chromosome 1(1309299), 3(1215399), 5(1090185), 7(980944), 9(732013), 11(815860), 13(602809), 15(491208), 17(477990), 19(393615) and 21(239352) with SNPs in it respectively.**
...
```{bash}
awk < /home/m280-data/hw1/merge-geno.bim '{if ($1 == 3 && $4 >= 47892180 && $4 <= 48130769) print}'|wc -l 
```
**Ans: There are 894 SNPs located within MAP4 gene.**
...
echo "    2.40  = FILE FORMAT VERSION NUMBER." > /home/nanchen322/Biostat-m280-2018-winter/hw1/mendel_snpdef.txt 
echo "8348674  = NUMBER OF SNPS LISTED HERE." >> /home/nanchen322/Biostat-m280-2018-winter/hw1/mendel_snpdef.txt 
awk '{OFS = ","} {print $2, $1, $4}' /home/m280-data/hw1/merge-geno.bim >> /home/nanchen322/Biostat-m280-2018-winter/hw1/mendel_snpdef.txt 
head mendel_snpdef.txt
```
**Comments: 1. add and append the first two lines 2. print col 2,1,4**
...
```{bash, echo=FALSE, eval=TRUE}
awk '{OFS = ","} {print $1, $2, $3, $4, $5, $6}' /home/m280-data/hw1/merge-geno.fam | awk -F, -v sex="" '{OFS = ","} {if ($5 == 1) sex="M"; else sex="F"} {print $1, $2, $3, $4, sex, $6}' |sed 's/T2DG//g' | awk -F, '{OFS = ","} {for (i = 1; i <= NF; i++) {$i = ($i == 0 ? "": $i)}; print}' > /home/nanchen322/Biostat-m280-2018-winter/hw1/mendel_ped.txt
head -20 mendel_ped.txt
```
**Comments: 1. change sex simbol 2. delete all lines T2DG 3. remove all zeros**
...
$$
for both methods.  
**Comments: Simulate data from three different distributions. Please see more details in runSim.R.**
...
**Comments: Read in data from runSim.R and run output to named txt files. Please see more details in autoSim.R.**
...
|     | SampAvg  |       |       |          |
```{bash, results = "asis"}
Rscript outputtable.R
```
**Comments: Read in data from named txt files. Please see more details in outputtable.R.**
